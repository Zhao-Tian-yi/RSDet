...
class RSDet_15th_2backbone(TwoStreamFasterRCNN):
    ...
    def _forward(self, batch_inputs: Tensor,
        ...
        # x_common, MI_loss = self.Gcommon(img_vis_unique, img_lwir_unique) #x_common concat了边缘信息
        x_common = []
        for i in range(len(x_vis)):
            x_common.append(0.5 * (x_vis[i] + x_lwir[i]))
        x_common = tuple(x_common)
        x_common = self.neck(x_common)
        # print(x_vis, x_lwir)
        # x,_,_ = self.FeaFusion(x_vis, x_lwir, img_vis,img_lwir)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # print(MI_loss,MI_loss_lwir,MI_loss_vis)
