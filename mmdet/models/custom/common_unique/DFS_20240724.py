...
class _Gate(nn.Module):
    def __init__(self,):
        ...
        # FLIR KAIST
        self.IA_fc1_1 = nn.Linear(in_features=32*40, out_features = 2)
        self.IA_fc2_1 = nn.Linear(in_features=16*20, out_features = 2)
        self.IA_fc3_1 = nn.Linear(in_features=80, out_features = 2)
        self.IA_fc4_1 = nn.Linear(in_features=20, out_features = 2)
        self.IA_fc5_1 = nn.Linear(in_features=20, out_features = 2)
        self.pool = nn.AvgPool2d(kernel_size=8) #FLIR KAIST
    ...
    def forward(self, x_vis, x_lwir):
        ...
        x3_1 = self.IA_fc1_1(x2_1)
        x3_2 = self.IA_fc2_1(x2_2)
        x3_3 = self.IA_fc3_1(x2_3)
        x3_4 = self.IA_fc4_1(x2_4)
        x3_5 = self.IA_fc5_1(x2_4)
...
class Conv11_Fusion4(BaseModule):
    def __init__(
            self,
            loss_MI,
            feature_nums,
        ...
        super(Conv11_Fusion4, self).__init__()
        self.expert_config = neck
        self.feature_nums = feature_nums
        self.channel_nums = channel_nums
        self.scale = scale
        self.num_gate =num_gate
        self.Gate = _Gate()
        self.expert_vis = MODELS.build(self.expert_config)
        self.expert_lwir = MODELS.build(self.expert_config)
    ...
    def forward(self, x_vis, x_lwir, x_common,img_vis,img_lwir):
        ...
        # weights.data[3,2,1]=0.03
        mask = weights < 0.05
        tensor = torch.where(mask, torch.tensor(0.0, device=weights.device), weights)
        gate = tensor.clone()

        gate[:, :, 1] = torch.where(tensor[:, :, 0] == 0, torch.tensor(1.0, device=tensor.device), gate[:, :, 1])
        gate[:, :, 0] = torch.where(tensor[:, :, 1] == 0, torch.tensor(1.0, device=tensor.device), gate[:, :, 0])

        x_vis_exclusive = self.expert_vis(x_vis)
        x_lwir_exclusive = self.expert_lwir(x_lwir)

        miloss1_vis = self.MILoss1(x_vis_exclusive[1] ,x_common[1])/(x_vis_exclusive[1].shape[2]*x_vis_exclusive[1].shape[3])
        miloss2_vis = self.MILoss2(x_vis_exclusive[2] ,x_common[2])/(x_vis_exclusive[2].shape[2]*x_vis_exclusive[2].shape[3])
        MIloss_vis =miloss1_vis+miloss2_vis
        miloss1_lwir = self.MILoss1(x_lwir_exclusive[1] ,x_common[1])/(x_vis_exclusive[1].shape[2]*x_vis_exclusive[1].shape[3])
        miloss2_lwir = self.MILoss2(x_lwir_exclusive[2] ,x_common[2])/(x_vis_exclusive[2].shape[2]*x_vis_exclusive[2].shape[3])
