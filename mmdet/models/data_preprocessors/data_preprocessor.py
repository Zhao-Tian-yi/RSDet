...
"""
@author：ty Zhao
@ClassName：_PairedDetDataPreprocessor
@Description:
"""

@MODELS.register_module()
class _PairedDetDataPreprocessor(BaseDataPreprocessor):
    def __init__(self,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
                 mean_lwir: Sequence[Number] = None,
                 std_lwir: Sequence[Number] = None,
                 pad_size_divisor: int = 1,
                 pad_value: Union[float, int] = 0,
                 pad_mask: bool = False,
                 mask_pad_value: int = 0,
        ...
        assert (mean_lwir is None) == (std_lwir is None), (
            'mean_lwir and std_lwir should be both None or tuple')
        if mean is not None:
        ...
        if mean_lwir is not None:
            assert len(mean_lwir) == 3 or len(mean_lwir) == 1, (
                '`mean_lwir` should have 1 or 3 values, to be compatible with '
                f'RGB or gray image, but got {len(mean)} values')
            assert len(std_lwir) == 3 or len(std_lwir) == 1, (  # type: ignore
                '`std_lwir` should have 1 or 3 values, to be compatible with RGB '  # type: ignore # noqa: E501
                f'or gray image, but got {len(std)} values')  # type: ignore
            self._enable_normalize_lwir = True
            self.register_buffer('mean_lwir',
                                 torch.tensor(mean_lwir).view(-1, 1, 1), False)
            self.register_buffer('std_lwir',
                                 torch.tensor(std_lwir).view(-1, 1, 1), False)
        else:
            self._enable_normalize_lwir = False
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        _batch_inputs = data['inputs']
        _batch_inputs_lwir = data['inputs_lwir']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_inputs = []
        ...
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs_lwir, torch.Tensor):
            batch_inputs_lwir = []
            for _batch_input_lwir in _batch_inputs_lwir:
                # channel transform
                if self._channel_conversion:
                    _batch_input_lwir = _batch_input_lwir[[2, 1, 0], ...]
                # Convert to float after channel conversion to ensure
                # efficiency
                _batch_input_lwir = _batch_input_lwir.float()
                # Normalization.
                if self._enable_normalize:
                    if self.mean.shape[0] == 3:
                        assert _batch_input_lwir.dim(
                        ) == 3 and _batch_input_lwir.shape[0] == 3, (
                            'If the mean has 3 values, the input tensor '
                            'should in shape of (3, H, W), but got the tensor '
                            f'with shape {_batch_input_lwir.shape}')
                    _batch_input_lwir = (_batch_input_lwir - self.mean_lwir) / self.std_lwir
                batch_inputs_lwir.append(_batch_input_lwir)
            # Pad and stack Tensor.
            batch_inputs_lwir = stack_batch(batch_inputs_lwir, self.pad_size_divisor,
                                       self.pad_value)
        # Process data with `default_collate`.
        elif isinstance(_batch_inputs_lwir, torch.Tensor):
            assert _batch_inputs_lwir.dim() == 4, (
                'The input of `ImgDataPreprocessor` should be a NCHW tensor '
                'or a list of tensor, but got a tensor with shape: '
                f'{_batch_inputs_lwir.shape}')
            if self._channel_conversion:
                _batch_inputs_lwir = _batch_inputs_lwir[:, [2, 1, 0], ...]
            # Convert to float after channel conversion to ensure
            # efficiency
            _batch_inputs_lwir = _batch_inputs_lwir.float()
            if self._enable_normalize:
                _batch_inputs_lwir = (_batch_inputs_lwir - self.mean_lwir) / self.std_lwir
            h, w = _batch_inputs_lwir.shape[2:]
            target_h = math.ceil(
                h / self.pad_size_divisor) * self.pad_size_divisor
            target_w = math.ceil(
                w / self.pad_size_divisor) * self.pad_size_divisor
            pad_h = target_h - h
            pad_w = target_w - w
            batch_inputs_lwir = F.pad(_batch_inputs_lwir, (0, pad_w, 0, pad_h),
                                      'constant', self.pad_value)
        else:
            raise TypeError('Output of `cast_data` should be a dict of '
                            'list/tuple with inputs and data_samples, '
        ...
        data['inputs_lwir'] = batch_inputs_lwir
...
"""
@author：Z.Tianyi
@ClassName：PairedDetDataPreprocessor
@Date：2023/9/18  21:00
@Description:
"""

@MODELS.register_module()
class PairedDetDataPreprocessor(_PairedDetDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # import pdb
        # pdb.set_trace()
        inputs, inputs_lwir, data_samples = data['inputs'], data['inputs_lwir'], data['data_samples']
        img_file = data['data_samples'][0].img_path
        if data_samples is not None:
            # NOTE the batched image size information may be useful, e.g.
            # in DETR, this is needed for the construction of masks, which is
            # then used for the transformer_head.
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                inputs, data_samples = batch_aug(inputs, data_samples)
                inputs_lwir, data_samples = batch_aug(inputs_lwir, data_samples)

        return {'inputs': {"img_vis":inputs,'img_lwir': inputs_lwir}, 'data_samples': data_samples,}
