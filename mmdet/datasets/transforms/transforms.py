...
@TRANSFORMS.register_module()
class PairedMosaic(BaseTransform):
    """Mosaic augmentation.
    ...
    def transform(self, results: dict) -> dict:
        ...
        if len(results['img'].shape) == 3:
            mosaic_img = np.full(
                ...
                self.pad_val,
                dtype=results['img'].dtype)
            mosaic_img_lwir = np.full(
                (int(self.img_scale[1] * 2), int(self.img_scale[0] * 2), 3),
                self.pad_val,
                dtype=results['img_lwir'].dtype)
        else:
            mosaic_img = np.full(
                (int(self.img_scale[1] * 2), int(self.img_scale[0] * 2)),
                self.pad_val,
        ...
        for i, loc in enumerate(loc_strs):
            ...
            img_i = results_patch['img']
            img_lwir_i = results_patch['img_lwir']
            h_i, w_i = img_i.shape[:2]
            # keep_ratio resize
            scale_ratio_i = min(self.img_scale[1] / h_i,
                                self.img_scale[0] / w_i)
            img_i = mmcv.imresize(
                img_i, (int(w_i * scale_ratio_i), int(h_i * scale_ratio_i)))
            img_lwir_i = mmcv.imresize(
                img_lwir_i, (int(w_i * scale_ratio_i), int(h_i * scale_ratio_i)))
            # compute the combine parameters
            paste_coord, crop_coord = self._mosaic_combine(
                loc, center_position, img_i.shape[:2][::-1])
            x1_p, y1_p, x2_p, y2_p = paste_coord
            ...
            # crop and paste image
            mosaic_img[y1_p:y2_p, x1_p:x2_p] = img_i[y1_c:y2_c, x1_c:x2_c]
            mosaic_img_lwir[y1_p:y2_p, x1_p:x2_p] = img_lwir_i[y1_c:y2_c, x1_c:x2_c]
            # adjust coordinate
            gt_bboxes_i = results_patch['gt_bboxes']
            gt_bboxes_labels_i = results_patch['gt_bboxes_labels']
            gt_ignore_flags_i = results_patch['gt_ignore_flags']
        ...
        results['img'] = mosaic_img
        results['img_lwir'] = mosaic_img_lwir
        results['img_shape'] = mosaic_img.shape[:2]
        results['gt_bboxes'] = mosaic_bboxes
        results['gt_bboxes_labels'] = mosaic_bboxes_labels
        results['gt_ignore_flags'] = mosaic_ignore_flags
...
@TRANSFORMS.register_module()
class PairedMixUp(BaseTransform):
    """MixUp data augmentation.
    ...
    def transform(self, results: dict) -> dict:
        ...
        retrieve_results = results['mix_results'][0]
        retrieve_img = retrieve_results['img']
        retrieve_img_lwir = retrieve_results['img_lwir']
        ...
        if len(retrieve_img.shape) == 3:
            out_img = np.ones(
                (self.dynamic_scale[1], self.dynamic_scale[0], 3),
                dtype=retrieve_img.dtype) * self.pad_val
            out_img_lwir = np.ones(
                (self.dynamic_scale[1], self.dynamic_scale[0], 3),
                dtype=retrieve_img_lwir.dtype) * self.pad_val
        else:
            out_img = np.ones(
                self.dynamic_scale[::-1],
                dtype=retrieve_img.dtype) * self.pad_val
        ...
        retrieve_img = mmcv.imresize(
            retrieve_img, (int(retrieve_img.shape[1] * scale_ratio),
                           int(retrieve_img.shape[0] * scale_ratio)))
        retrieve_img_lwir = mmcv.imresize(
            retrieve_img_lwir, (int(retrieve_img_lwir .shape[1] * scale_ratio),
                           int(retrieve_img_lwir .shape[0] * scale_ratio)))
        # 2. paste
        out_img[:retrieve_img.shape[0], :retrieve_img.shape[1]] = retrieve_img
        out_img_lwir [:retrieve_img_lwir .shape[0], :retrieve_img_lwir .shape[1]] = retrieve_img_lwir

        # 3. scale jit
        scale_ratio *= jit_factor
        out_img = mmcv.imresize(out_img, (int(out_img.shape[1] * jit_factor),
                                          int(out_img.shape[0] * jit_factor)))
        out_img_lwir = mmcv.imresize(out_img_lwir, (int(out_img_lwir.shape[1] * jit_factor),
                                          int(out_img_lwir.shape[0] * jit_factor)))
        # 4. flip
        if is_filp:
            out_img = out_img[:, ::-1, :]
            out_img_lwir  = out_img_lwir [:, ::-1, :]
        # 5. random crop
        ori_img = results['img']
        ori_img_lwir = results['img_lwir']
        origin_h, origin_w = out_img.shape[:2]
        target_h, target_w = ori_img.shape[:2]
        padded_img = np.ones((max(origin_h, target_h), max(
            origin_w, target_w), 3)) * self.pad_val
        padded_img_lwir  = np.ones((max(origin_h, target_h), max(
            origin_w, target_w), 3)) * self.pad_val

        padded_img = padded_img.astype(np.uint8)
        padded_img_lwir = padded_img_lwir.astype(np.uint8)
        padded_img[:origin_h, :origin_w] = out_img
        padded_img_lwir [:origin_h, :origin_w] = out_img_lwir
        ...
        padded_cropped_img = padded_img[y_offset:y_offset + target_h,
                                        x_offset:x_offset + target_w]
        padded_cropped_img_lwir = padded_img_lwir[y_offset:y_offset + target_h,
                                        x_offset:x_offset + target_w]
        # 6. adjust bbox
        retrieve_gt_bboxes = retrieve_results['gt_bboxes']
        retrieve_gt_bboxes.rescale_([scale_ratio, scale_ratio])
        ...
        # 8. mix up
        ori_img = ori_img.astype(np.float32)
        ori_img_lwir = ori_img_lwir.astype(np.float32)
        mixup_img = 0.5 * ori_img + 0.5 * padded_cropped_img.astype(np.float32)
        mixup_img_lwir = 0.5 * ori_img_lwir + 0.5 * padded_cropped_img_lwir.astype(np.float32)
        ...
        results['img'] = mixup_img.astype(np.uint8)
        results['img_lwir'] = mixup_img_lwir.astype(np.uint8)
        results['img_shape'] = mixup_img.shape[:2]
        results['gt_bboxes'] = mixup_gt_bboxes
        results['gt_bboxes_labels'] = mixup_gt_bboxes_labels
        results['gt_ignore_flags'] = mixup_gt_ignore_flags
...
from mmcv.transforms import MultiScaleFlipAug,TestTimeAug
"""
@author：ty Zhao
"""
@TRANSFORMS.register_module()
