...
class TwoStreamFasterRCNNCommonUnique_2nd(TwoStreamFasterRCNN):
    ...
    def extract_lwirfeat(self, img):
        ...
        if self.with_neck:
            x = self.neck_lwir(x)
        return x
    def extract_fusion(self, img):
        """Directly extract features from the backbone+neck."""
        x = self.backbone_fusion(img)
        if self.with_neck:
            x = self.neck_fusion(x)
        return x
    def _forward(self, batch_inputs: Tensor,
        ...
        x_common,_ = self.Gcommon(img_vis,img_lwir)
        x_common = self.extract_fusion(x_common)
        # mask_vis, mask_lwir = self.Gmask(img_vis, img_lwir)

        x = []
        for i in range(len(x_vis)):
            x.append(0.33*(x_common[i]+ x_vis[i] + x_lwir[i]))
        x = tuple(x)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        x_vis = self.extract_visfeat(img_vis )
        x_lwir = self.extract_lwirfeat(img_lwir)
        x_common,MI_loss_value = self.Gcommon(img_vis,img_lwir)

        x_common = self.extract_fusion(x_common)
        MI_loss = {'loss_MI': 0.0001*MI_loss_value}
