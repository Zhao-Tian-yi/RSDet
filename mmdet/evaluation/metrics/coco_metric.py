...
class CocoMetric(BaseMetric):
    ...
    def save_results2txt(self, results: Sequence[dict],
                     outfile_prefix: str) -> dict:
        """Dump the detection results to a txt files.
        ...
        """
        ann_path = 'Datasets/kaist-paired/kaist_test_data.json'
        tr = json.load(open(ann_path))
        outfile_prefix = './'
        sava_root=outfile_prefix+'/det/'
        score_thr=0.01
        if not os.path.exists(sava_root):
            os.mkdir(sava_root)
        id_name_dict ={}
        for img_info in tr['images']:
            txt_name = img_info['file_name'][:-12]+".txt"
            # f= open(sava_root+txt_name,'w')
            # f.close()
            id_name_dict[str(img_info['id'])] =  txt_name
        ...
        for idx, result in enumerate(results):
            ...
            txt_name = id_name_dict[str(image_id)]
            # bbox results
            with open(sava_root+txt_name,'w') as txt:
                for i, label in enumerate(labels):
                    if scores[i] < score_thr:
                        continue
                    sx, sy, ex, ey = bboxes[i][0], bboxes[i][1], bboxes[i][2], bboxes[i][3]
                    line = 'person ' + str(sx) + ' ' + str(sy) + ' ' + str(ex) + ' ' + str(ey) + ' ' + str(scores[i]) + '\n'
                    txt.write(line)
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # convert predictions to txt files
        # self.save_results2txt(preds,outfile_prefix)
