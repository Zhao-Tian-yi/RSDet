...
def format_params_in_m(params):
    """Format parameters in millions (M)"""
    return f"{params / 1e6:.2f}M"


def format_flops_in_g(flops):
    """Format FLOPs in billions (G)"""
...
def inference(args, logger):
    ...
    # 使用自定义格式化函数
    mean_flops = format_flops_in_g(int(np.average(avg_flops)))
    params_formatted = format_params_in_m(params)
    result['flops'] = mean_flops
    result['params'] = params_formatted
